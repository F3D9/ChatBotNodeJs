...
async function userAlreadyCreated(user, email) {
    const resultUserName = await dbUsuarios.query('SELECT username FROM users WHERE username = $1 or email = $2', [user, email]);
    if (resultUserName.rows.length > 0)
        return true;
    return false;
}
async function createUserDataBase(user, email, password) {
    await dbUsuarios.query('INSERT INTO users(username,email,password) VALUES ($1,$2,$3)', [user, email, password]);
}
async function emailAlreadyRegisted(email) {
    const resultEmail = await dbUsuarios.query('SELECT username,password FROM users WHERE email = $1', [email]);
    return resultEmail;
}
async function deleteUserQuery(email) {
    const resultEmail = await dbUsuarios.query('DELETE FROM users WHERE email = $1', [email]);
    return resultEmail;
}
async function login(req, res) {
    ...
    if (userPassword.username.length == 0)
    ...
    if (!loginCorrecto)
        return res.status(400).send({ status: "Error", message: "Los campos estan mal" });
    const token = jsonwebtoken.sign({ user: userPassword.rows[0].username }, process.env.JWT_SECRET, { expiresIn: process.env.JWT_EXPIRATION });
    const cookieOption = {
